...
def get_model(configs):
    if configs.model.model_name.startswith("vit"):
        ...
        if configs.model.local_pretrained:
            if 'outputs' in configs.model.local_pretrained_path:
                state_dict = torch.load(configs.model.local_pretrained_path).state_dict()
            else:
                state_dict = torch.load(configs.model.local_pretrained_path)
            state_dict.pop('fc.weight', None)
            state_dict.pop('fc.bias', None)
            state_dict.pop('classifier.weight', None)
            state_dict.pop('classifier.bias', None)
            missing, unexpected = model.load_state_dict(state_dict, strict=False)
            print(f'load model from local, missing: {missing}; unexpected: {unexpected}')
