...
#     #   L_G : maximize I(Z;Y)   via CE-like Z-to-class alignment
#     #   L_R : maximize I(Zr;Y)  via CE-like Zr-to-class alignment
#     #   L_N : log1p(regularized empirical NOCCO(Zn, one_hot(Y))) by default.
#     #         nhsic_objective='raw' selects the untransformed trace control.
#     #         Population information bound requires the stated RKHS assumptions.
#     loss_region: Tensor
#     loss_global: Tensor
#     loss_relevant: Tensor
#     loss_nuisance: Tensor
...
#     Objectives
#     ----------
#         L_G = CE(cosine(Z, W)/tau + log prior, y)     [unchanged]
#         L_R = CE(cosine(Zr, W)/tau + log prior, y)    [unchanged]
#         N_hat = tr[Kc(Kc+n*rho*I)^-1 Lc(Lc+n*rho*I)^-1]
#         L_N = log(1 + N_hat)  [default]; or N_hat with nhsic_objective='raw'

#     L_N uses raw pooled nuisance vectors from EVERY image (zero for an empty
#     nuisance set), a fixed-bandwidth RBF feature kernel and a one-hot linear
#     label kernel. There is no per-vector L2 normalization or valid-row filtering.
#     This is operator-normalized HSIC / empirical NOCCO, not CKA. Under suitable
#     population RKHS assumptions, NOCCO equals Pearson chi-square dependence and
#     I(Zn;Y) <= log(1 + NOCCO) <= NOCCO. A fixed-ridge minibatch statistic is NOT
#     itself a certified upper bound on population mutual information.

#     Sakamoto & Sato (TMLR 2024), Appendix D.3, Eqs. (22)-(23), gives the
#     population information-theoretic link. Appendix F.1.2 sets sigma=5*sqrt(d)
#     and rho=1e-5 for the empirical estimator. We adopt those defaults. Using
#     log1p(N_hat) as a nuisance TRAINING loss is our adaptation of their bound;
#     their paper uses nHSIC to analyze trained representations. This is not a
#     reproduction of their whole training method or an exact KL marginal bound.
#     https://arxiv.org/pdf/2402.09050
#     Original NOCCO theory: Fukumizu et al., NeurIPS 2007.
...
#         clustering_metric: str = "cosine",
#         *,
#         nhsic_regularization: float = 1e-5,
#         nhsic_bandwidth: Optional[float] = None,
#         nhsic_calibration_max_samples: int = 512,
#         nhsic_objective: str = "log1p",
#         nhsic_bandwidth_mode: str = "paper",
#         nhsic_restore_config: bool = False,
#     ) -> None:
#         if num_classes < 2:
#             raise ValueError("num_classes must be >= 2.")
#         if not (0.0 <= float(decision_threshold) < 1.0):
...
#         self.nhsic_objective = str(nhsic_objective).strip().lower()
#         if self.nhsic_objective not in ("log1p", "raw"):
#             raise ValueError("nhsic_objective must be 'log1p' or 'raw'.")
#         self.nhsic_bandwidth_mode = str(nhsic_bandwidth_mode).strip().lower()
#         if self.nhsic_bandwidth_mode not in ("paper", "median"):
#             raise ValueError("nhsic_bandwidth_mode must be 'paper' or 'median'.")
#         # Default: reuse saved banks with THIS experiment's constructor settings.
#         # Opt in to restoring saved nuisance settings when resuming an exact run.
#         self.nhsic_restore_config = bool(nhsic_restore_config)

#         # This ridge is distinct from lambda_nuisance (the outer loss weight).
#         self.nhsic_regularization = float(nhsic_regularization)
#         if not math.isfinite(self.nhsic_regularization) or self.nhsic_regularization <= 0:
#             raise ValueError("nhsic_regularization must be finite and > 0.")
#         if nhsic_bandwidth is not None:
#             nhsic_bandwidth = float(nhsic_bandwidth)
#             if not math.isfinite(nhsic_bandwidth) or nhsic_bandwidth <= 0:
#                 raise ValueError("nhsic_bandwidth (RBF sigma) must be finite and > 0.")
#         if int(nhsic_calibration_max_samples) < 2:
#             raise ValueError("nhsic_calibration_max_samples must be >= 2.")
#         self.nhsic_calibration_max_samples = int(nhsic_calibration_max_samples)
#         self._nhsic_requested_bandwidth = nhsic_bandwidth
#         self.nhsic_bandwidth = nhsic_bandwidth
#         self.nhsic_bandwidth_source = "manual" if nhsic_bandwidth is not None else "pending"
...
#             "max_relative_score": float(relative_score.max().item()),
#         }
#         # Reuse this discovery pass; reference_loader need not be reiterable.
#         # These vectors only set a fixed kernel scale, not the Stage-II masks.
#         if self.nhsic_bandwidth is None:
#             pooled_nuisance, _ = self._masked_avg_pool(regions, nuisance_mask_all)
#             stats["_nhsic_nuisance_pooled"] = pooled_nuisance.detach().float().cpu()
#         return relevant_by_class, nuisance_by_class, irrelevant_by_class, stats
...
#         self.nhsic_bandwidth = self._nhsic_requested_bandwidth
#         self.nhsic_bandwidth_source = "manual" if self.nhsic_bandwidth is not None else "pending"
#         bandwidth_chunks: List[Tensor] = []
#         bandwidth_count = 0
#         if self.nhsic_bandwidth is None and self.nhsic_bandwidth_mode == "paper":
#             feature_dim = int(_get_classifier(model).weight.shape[1])
#             self._calibrate_nhsic_bandwidth(
#                 torch.empty(0, feature_dim), source="discovery_feature_dimension"
#             )
...
#             if self.nhsic_bandwidth is None and bandwidth_count < self.nhsic_calibration_max_samples:
#                 chunk = stats["_nhsic_nuisance_pooled"]
#                 chunk = chunk[:self.nhsic_calibration_max_samples - bandwidth_count]
#                 bandwidth_chunks.append(chunk)
#                 bandwidth_count += int(chunk.shape[0])
...
#         if self.nhsic_bandwidth is None:
#             self._calibrate_nhsic_bandwidth(
#                 torch.cat(bandwidth_chunks, dim=0), source="discovery_pooled_nuisance"
#             )
#         if verbose:
#             print("[TMLR-D3] objective={}, fixed sigma={:.6g}, rho={:.6g}, source={}".format(
#                 self.nhsic_objective, self.nhsic_bandwidth,
#                 self.nhsic_regularization, self.nhsic_bandwidth_source
#             ))
...
#     # -------------------------------------------------------------------------
#     # Operator-normalized HSIC / NOCCO on raw Zn and one-hot labels
#     # -------------------------------------------------------------------------

#     @staticmethod
#     def _nhsic_squared_distances(x: Tensor) -> Tensor:
#         # Translation leaves RBF distances unchanged and reduces cancellation.
#         # This does NOT normalize individual vectors or discard their magnitudes.
#         shifted = x - x.mean(dim=0, keepdim=True)
#         x2 = shifted.square().sum(dim=1, keepdim=True)
#         dist2 = (x2 + x2.t() - 2.0 * (shifted @ shifted.t())).clamp_min(0.0)
#         return dist2 - torch.diag_embed(dist2.diagonal())

#     @torch.no_grad()
#     def _calibrate_nhsic_bandwidth(self, x: Tensor, source: str) -> None:
#         """Fix sigma once: 5*sqrt(d) by default; optional previous median control."""
#         if self.nhsic_bandwidth is not None:
#             return
#         if x.ndim != 2 or x.shape[1] < 1:
#             raise ValueError("Bandwidth setup requires [N,D] with D >= 1.")
#         if self.nhsic_bandwidth_mode == "paper":
#             # Appendix F.1.2: d is the dimension of the vector entering the kernel.
#             # Here that vector is pooled Zn, not the unpooled H*W*D feature map.
#             self.nhsic_bandwidth = 5.0 * math.sqrt(int(x.shape[1]))
#             self.nhsic_bandwidth_source = "paper_5sqrt_d"
#             return
#         x = x[:self.nhsic_calibration_max_samples].detach().to(device="cpu", dtype=torch.float64)
#         if x.ndim != 2 or not bool(torch.isfinite(x).all()):
#             raise ValueError("Bandwidth calibration requires finite [N,D] features.")
#         sigma = 1.0
#         calibrated = False
#         if x.shape[0] >= 2:
#             dist2 = self._nhsic_squared_distances(x)
#             if not bool(torch.isfinite(dist2).all()):
#                 raise ValueError("Non-finite squared distances in bandwidth calibration.")
#             upper = torch.triu_indices(x.shape[0], x.shape[0], offset=1)
#             pairs = dist2[upper[0], upper[1]]
#             positive = pairs[pairs > 0]
#             if positive.numel():
#                 sigma = math.sqrt(float(positive.median().item()))
#                 calibrated = True
#         self.nhsic_bandwidth = sigma
#         self.nhsic_bandwidth_source = source if calibrated else source + "_fallback_1"

#     def _rbf_kernel_median(self, x: Tensor) -> Tensor:
#         """Compatibility name: fixed RBF, default sigma=5*sqrt(d), without L2."""
#         if self.nhsic_bandwidth is None:
#             # Normal public use calibrates in discover/load_discovery. This path
#             # keeps direct private-helper use possible without changing its API.
#             self._calibrate_nhsic_bandwidth(x, source="first_direct_call")
#         with torch.autocast(device_type=x.device.type, enabled=False):
#             scaled = x.to(dtype=torch.float64) / float(self.nhsic_bandwidth)
#             dist2 = self._nhsic_squared_distances(scaled)
#             if not bool(torch.isfinite(dist2).all()):
#                 raise ValueError("Non-finite RBF distances; check feature scale and bandwidth.")
...
#     def _normalized_hsic(self, x: Tensor, y: Tensor) -> Tensor:
#         """
#         Raw regularized empirical NOCCO (TMLR 2024, Appendix F.1.2).

#         x: raw pooled features; y: one-hot labels, used with a LINEAR kernel.
#         Kc = H RBF(x,x) H, Lc = H (y y^T) H, ridge = n * lambda.
#         Return tr[Kc(Kc+ridge*I)^-1 Lc(Lc+ridge*I)^-1].
#         No CKA denominator, L2 preprocessing, log transform or clipping to one.
#         The objective transform is applied by _nuisance_to_class_nhsic_loss.
#         Symmetric double centering gives the same trace as the paper's KH/LH
#         expression, and permits numerically stable Cholesky factorizations.
#         This finite-batch, positive-ridge value is not an exact MI estimate or
#         a certified upper bound on population MI. It may exceed one.
#         """
#         if x.ndim != 2 or y.ndim != 2 or x.shape[0] != y.shape[0]:
#             raise ValueError("x,y must be [N,Dx]/[N,Dy] with the same N.")
...
#             raise ValueError("x must contain floating-point features.")
#         if not bool(torch.isfinite(x).all()) or not bool(torch.isfinite(y).all()):
#             raise ValueError("NOCCO inputs must be finite.")
#         n = int(x.shape[0])
#         output_dtype = torch.float64 if x.dtype == torch.float64 else torch.float32
#         if n < 2:
#             return (x.to(output_dtype).sum() + y.to(output_dtype).sum()) * 0.0

#         # FP64 solves avoid AMP/FP16 centering and factorization instability.
#         # Casting is differentiable: gradients still flow to the student features.
#         with torch.autocast(device_type=x.device.type, enabled=False):
#             xd = x.to(torch.float64)
#             yd = y.to(torch.float64)
#             if bool((xd == xd[:1]).all()) or bool((yd == yd[:1]).all()):
#                 return ((xd.sum() + yd.sum()) * 0.0).to(output_dtype)
#             kx = self._center_kernel(self._rbf_kernel_median(xd))
#             ky = self._center_kernel(yd @ yd.t())
#             kx = 0.5 * (kx + kx.t())
#             ky = 0.5 * (ky + ky.t())
#             identity = torch.eye(n, dtype=torch.float64, device=x.device)
#             ridge = n * self.nhsic_regularization
#             cx, info_x = torch.linalg.cholesky_ex(kx + ridge * identity)
#             cy, info_y = torch.linalg.cholesky_ex(ky + ridge * identity)
#             if int(info_x.item()) != 0 or int(info_y.item()) != 0:
#                 raise RuntimeError(
#                     "NOCCO Cholesky failed. Check features or increase "
#                     "nhsic_regularization; no unreported jitter is added."
#                 )
#             # (Kc+aI)^-1 Kc = Kc(Kc+aI)^-1, since both commute.
#             ax = torch.cholesky_solve(kx, cx)
#             ay = torch.cholesky_solve(ky, cy)
#             nhsic = (ax * ay.t()).sum()
#             if not bool(torch.isfinite(nhsic)):
#                 raise RuntimeError("Non-finite NOCCO loss.")
#             # The exact trace is nonnegative; only remove negative roundoff.
#             return nhsic.clamp_min(0.0).to(output_dtype)
...
#     ) -> Tensor:
#         """
#         L_N = log1p(empirical NOCCO(Zn, one_hot(Y))) by default, over all images.
#         With nhsic_objective='raw', use the same statistic without log1p.

#         Empty nuisance sets are represented by zero vectors and remain included.
#         valid_nuisance and classifier stay in the signature for caller compatibility.
#         The label kernel no longer depends on classifier directions or their rank.
#         """
#         if z_nuisance.ndim != 2:
#             raise ValueError("z_nuisance must be [B,D].")
#         n = z_nuisance.shape[0]
#         if valid_nuisance.shape != (n,) or labels.shape != (n,):
#             raise ValueError("valid_nuisance and labels must be [B].")
#         y = labels.to(device=z_nuisance.device, dtype=torch.long)
#         if bool(((y < 0) | (y >= self.num_classes)).any()):
#             raise ValueError("labels must be in [0, num_classes).")
#         valid = valid_nuisance.to(device=z_nuisance.device, dtype=torch.bool)
#         zn = torch.where(valid.unsqueeze(1), z_nuisance, torch.zeros_like(z_nuisance))
#         one_hot = F.one_hot(y, num_classes=self.num_classes).to(dtype=torch.float32)
#         nocco = self._normalized_hsic(zn, one_hot)
#         return torch.log1p(nocco) if self.nhsic_objective == "log1p" else nocco
...
#         self._check_discovered()
#         payload = {
#             "version": "triple_bank_rni_tmlr_d3_v7",
#             "nhsic": {
#                 "estimator": "regularized_nocco_trace",
#                 "objective": self.nhsic_objective,
#                 "bandwidth_mode": self.nhsic_bandwidth_mode,
#                 "reference": "Sakamoto_Sato_TMLR_2024_D3_F1.2",
#                 "feature_kernel": "rbf_raw",
#                 "label_kernel": "one_hot_linear",
#                 "sample_scope": "all_images_zero_for_empty",
#                 "regularization": self.nhsic_regularization,
#                 "bandwidth": self.nhsic_bandwidth,
#                 "requested_bandwidth": self._nhsic_requested_bandwidth,
#                 "bandwidth_source": self.nhsic_bandwidth_source,
#                 "calibration_max_samples": self.nhsic_calibration_max_samples,
#             },
#             "clustering_metric": self.clustering_metric,
#             "num_classes": self.num_classes,
#             "prototype_k_factors": self.prototype_k_factors,
...
#             "irrelevant_best_k": self.irrelevant_best_k,
#             "discovery_result": (
#                 None if self.discovery_result is None else asdict(self.discovery_result)
#             ),
#         }
#         os.makedirs(os.path.dirname(os.path.abspath(path)), exist_ok=True)
#         torch.save(payload, path)
...
#         device: Union[str, torch.device],
#     ) -> None:
#         # New files contain only tensors and ordinary Python containers. Allow
#         # just our result class as well, for original v5 dataclass checkpoints.
#         if hasattr(torch.serialization, "safe_globals"):
#             with torch.serialization.safe_globals([DualPatternDiscoveryResult]):
#                 payload = torch.load(path, map_location="cpu", weights_only=True)
#         else:
#             # PyTorch 2.0-2.3: retain the original loader for the user's own v5
#             # checkpoints. As in the original script, these files must be trusted.
...
#         self.irrelevant_best_k = int(payload["irrelevant_best_k"])
#         self.discovery_result = payload.get("discovery_result", None)
#         if isinstance(self.discovery_result, dict):
#             self.discovery_result = DualPatternDiscoveryResult(**self.discovery_result)

#         nhsic_config = payload.get("nhsic")
#         if nhsic_config is not None and self.nhsic_restore_config:
#             if nhsic_config.get("estimator") != "regularized_nocco_trace":
#                 raise ValueError("Unsupported nuisance estimator in saved discovery.")
#             regularization = float(nhsic_config["regularization"])
#             bandwidth = nhsic_config.get("bandwidth")
#             if not math.isfinite(regularization) or regularization <= 0:
#                 raise ValueError("Saved NOCCO regularization must be finite and > 0.")
#             if bandwidth is not None and (not math.isfinite(float(bandwidth)) or float(bandwidth) <= 0):
#                 raise ValueError("Saved RBF bandwidth must be finite and > 0.")
#             self.nhsic_regularization = regularization
#             objective = str(nhsic_config.get("objective", "raw"))
#             bandwidth_mode = str(nhsic_config.get("bandwidth_mode", "median"))
#             if objective not in ("log1p", "raw") or bandwidth_mode not in ("paper", "median"):
#                 raise ValueError("Saved NOCCO objective/bandwidth mode is unsupported.")
#             self.nhsic_objective = objective
#             self.nhsic_bandwidth_mode = bandwidth_mode
#             self.nhsic_bandwidth = None if bandwidth is None else float(bandwidth)
#             self._nhsic_requested_bandwidth = nhsic_config.get("requested_bandwidth")
#             self.nhsic_bandwidth_source = nhsic_config.get("bandwidth_source", "checkpoint")
#             self.nhsic_calibration_max_samples = int(nhsic_config.get("calibration_max_samples", 512))
#             if self.nhsic_calibration_max_samples < 2:
#                 raise ValueError("Saved calibration_max_samples must be >= 2.")
#         else:
#             # Reuse banks with the current experiment's settings (default), or
#             # load a v5 file which has no nuisance configuration to restore.
#             # This prevents loading an old bank from silently undoing D.3 defaults.
#             self.nhsic_bandwidth = self._nhsic_requested_bandwidth
#             self.nhsic_bandwidth_source = "manual" if self.nhsic_bandwidth is not None else "pending"
#         if self.nhsic_bandwidth is None:
#             self._calibrate_nhsic_bandwidth(
#                 self.nuisance_medoids_raw, source="loaded_nuisance_medoids"
#             )
...
class DualPatternResNet(object):
    ...
    def _extract_decision_aware_candidates(
        ...
        Thus every patch belongs to exactly one discovery pool.
        """
        logits, _, features = _capture_layer4_and_forward(model, inputs)
        regions = self._regions_from_features(features)  # [B,R,D]
        labels = labels.long().to(logits.device)
    ...
    @torch.no_grad()
    def evaluate_bank_nuisance_removal(
        self,
        student_model: nn.Module,
        inputs: Union[Tensor, Iterable],
        labels: Optional[Tensor] = None,
        save_dir: Optional[str] = None,
        class_groups: Optional[Dict[str, Sequence[int]]] = None,
        verbose: bool = True,
    ) -> Dict[str, Any]:
        """
        Evaluate zeroing ALL patches assigned to N by the existing R/N/I banks.

        Run discover(model, dataloaders["val"], device) first, or reuse an
        already-discovered instance. Matching ALWAYS uses its frozen discovery
        model, preserving the bank feature space. The target student must have
        the same layer4 spatial grid. No test label is used to generate masks.

        Inputs follow _batch_iterator: a loader yields (images, ..., labels), or
        pass an image Tensor plus labels. All samples, including those without
        nuisance patches, enter accuracy denominators. Zeroing is at layer4,
        followed by the model's original pooling/head; there is NO renormalizing
        by the number of remaining patches and NO additional logit adjustment.

        class_groups optionally supplies the experiment's existing class IDs,
        e.g. {"Head": [...], "Medium": [...], "Tail": [...]}; none are inferred.
        Accuracies/patch percentages are on [0,100], gains in percentage points.
        Absent classes/groups have None accuracy, not zero. Optional CSVs contain
        overall, per-class, per-group, and per-image results.
        This method never rebuilds banks, sweeps thresholds, or trains a model.
        """
        self._check_discovered()
        assert self._assignment_model is not None
        try:
        ...
        seen_classes = set()
        for name, class_ids in (class_groups or {}).items():
            ids = list(class_ids)
            if any(int(c) != c or not 0 <= int(c) < self.num_classes for c in ids):
                raise ValueError("class_groups must contain valid integer class IDs.")
            ids = [int(c) for c in ids]
            if len(set(ids)) != len(ids) or seen_classes.intersection(ids):
                raise ValueError("class_groups must not contain overlapping class IDs.")
            groups[str(name)] = ids
            seen_classes.update(ids)

        assignment_model = self._assignment_model
        # Preserve mixed module modes (e.g. train-mode model with frozen BN).
        saved_modes = {
            module: module.training
            for root in (student_model, assignment_model)
        ...
        try:
            ...
            for batch_inputs, batch_labels in self._batch_iterator(inputs, labels):
                ...
                if x.ndim != 4 or x.shape[0] == 0:
                    raise ValueError("Expected a nonempty image batch [B,C,H,W].")

                # Record the exact discovery grid without another model forward.
                assignment_grid: List[Tuple[int, int]] = []

                def _capture_grid(_module, _inputs, output):
                    if not torch.is_tensor(output) or output.ndim != 4:
                        raise ValueError("Assignment layer4 must output [B,D,H,W].")
                    assignment_grid.append(tuple(output.shape[-2:]))

                handle = _get_layer4(assignment_model).register_forward_hook(_capture_grid)
                try:
                    # ONLY images enter matching: no GT or GT-selected bank.
                    partition = self._stage2_triple_bank_partition(x)
                finally:
                    handle.remove()

                logits_before, _, features = _capture_layer4_and_forward(student_model, x)
                b = int(x.shape[0])
                grid = tuple(features.shape[-2:])
                if len(assignment_grid) != 1 or assignment_grid[0] != grid:
                    raise ValueError(
                        "Frozen assignment and student layer4 grids must match: "
                        "{} versus {}.".format(assignment_grid, grid)
                    )
                remove_mask = partition["nuisance_mask"].to(device=device, dtype=torch.bool)
                num_patches = int(grid[0] * grid[1])
                if remove_mask.shape != (b, num_patches):
                    raise ValueError("Bank nuisance mask does not match the student patch grid.")
                if logits_before.shape != (b, self.num_classes):
                    raise ValueError("Student logits must have shape [B,num_classes].")

                # Keep the original forward path, including GAP denominator P.
                logits_after = (
                    _forward_with_layer4_zero_mask(student_model, x, remove_mask)
                    if bool(remove_mask.any())
                    else logits_before
                )
                if logits_after.shape != logits_before.shape:
                    raise ValueError("Intervention changed the logit shape.")
                if not bool(torch.isfinite(logits_before).all()) or not bool(
                    torch.isfinite(logits_after).all()
                ):
                    raise ValueError("Non-finite logits encountered during evaluation.")

                pred_before = logits_before.argmax(dim=1).cpu()
                pred_after = logits_after.argmax(dim=1).cpu()
                removed_counts = remove_mask.sum(dim=1).cpu()

                # Labels are read only AFTER both sets of predictions exist.
                if not torch.is_tensor(batch_labels) or batch_labels.shape != (b,):
                    raise ValueError("Labels must be a Tensor with shape [B].")
                y = batch_labels.detach().cpu()
                if y.is_complex() or not bool(torch.isfinite(y).all()):
                    raise ValueError("Labels must be finite integer class IDs.")
                if bool((y != y.long()).any()):
                ...
                for i in range(b):
                    target = int(y[i])
                    before, after = int(pred_before[i]), int(pred_after[i])
                    count = int(removed_counts[i])
                    records.append({
                        "sample_index": start + i,
                        "label": target,
                        "prediction_before": before,
                        "prediction_after": after,
                        "correct_before": int(before == target),
                        "correct_after": int(after == target),
                        "num_patches": num_patches,
                        "num_nuisance_patches": count,
                        "nuisance_patch_ratio_pct": 100.0 * count / num_patches,
                    })
        finally:
        ...
        if not records:
            raise ValueError("No samples were found in the supplied inputs/loader.")

        def _summarize(rows: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
            n = len(rows)
            cb = sum(r["correct_before"] for r in rows)
            ca = sum(r["correct_after"] for r in rows)
            patches = sum(r["num_patches"] for r in rows)
            removed = sum(r["num_nuisance_patches"] for r in rows)
            per_class_accuracy_before: List[float] = []
            per_class_accuracy_after: List[float] = []
            for c in sorted({r["label"] for r in rows}):
                cr = [r for r in rows if r["label"] == c]
                per_class_accuracy_before.append(
                    100.0 * sum(r["correct_before"] for r in cr) / len(cr)
                )
                per_class_accuracy_after.append(
                    100.0 * sum(r["correct_after"] for r in cr) / len(cr)
                )
            macro_before = (
                sum(per_class_accuracy_before) / len(per_class_accuracy_before) if n else None
            )
            macro_after = (
                sum(per_class_accuracy_after) / len(per_class_accuracy_after) if n else None
            )
            return {
                "num_images": n,
                "num_present_classes": len(per_class_accuracy_before),
                "num_correct_before": cb,
                "num_correct_after": ca,
                "accuracy_before": 100.0 * cb / n if n else None,
                "accuracy_after": 100.0 * ca / n if n else None,
                "accuracy_gain_pp": 100.0 * (ca - cb) / n if n else None,
                "macro_accuracy_before": macro_before,
                "macro_accuracy_after": macro_after,
                "macro_accuracy_gain_pp": macro_after - macro_before if n else None,
                "num_corrected": sum(
                    not r["correct_before"] and r["correct_after"] for r in rows
                ),
                "num_harmed": sum(
                    r["correct_before"] and not r["correct_after"] for r in rows
                ),
                "num_prediction_changed": sum(
                    r["prediction_before"] != r["prediction_after"] for r in rows
                ),
                "num_images_with_nuisance": sum(
                    r["num_nuisance_patches"] > 0 for r in rows
                ),
                "num_patches": patches,
                "num_nuisance_patches": removed,
                "nuisance_patch_ratio_pct": 100.0 * removed / patches if patches else None,
            }

        summary = _summarize(records)
        summary.update({
            "mask_source": "frozen_discovery_model_nearest_R_N_I_bank",
            "clustering_metric": self.clustering_metric,
            "discovery_decision_threshold": self.decision_threshold,
        })
        per_class = [
            {"class_id": c, **_summarize([r for r in records if r["label"] == c])}
            for c in range(self.num_classes)
        ]
        per_group = [
            {"group": name, **_summarize([r for r in records if r["label"] in ids])}
        ...
        if save_dir is not None:
            ...
            for name, rows in (
                ...
                if not rows:
                    continue
                path = os.path.abspath(os.path.join(save_dir, "bank_remove_n_" + name + ".csv"))
        ...
        if verbose:
            print(
                "[Bank Remove-N] images={} | accuracy: {:.4f}% -> {:.4f}% "
                "| gain={:+.4f} pp".format(
                    summary["num_images"], summary["accuracy_before"],
                    summary["accuracy_after"], summary["accuracy_gain_pp"]
                )
            )
            print(
                "[Bank Remove-N] macro accuracy: {:.4f}% -> {:.4f}% | "
                "nuisance patches={:.4f}% | with N={}/{} | corrected={} | harmed={}".format(
                    summary["macro_accuracy_before"], summary["macro_accuracy_after"],
                    summary["nuisance_patch_ratio_pct"], summary["num_images_with_nuisance"],
                    summary["num_images"], summary["num_corrected"], summary["num_harmed"]
                )
            )
            print("[Bank Remove-N] Per-class accuracy (correct / class sample count):")
            for row in per_class:
                if row["num_images"]:
                    print(
                        "[Bank Remove-N] class={} | images={} | accuracy: {:.4f}% -> {:.4f}% "
                        "| gain={:+.4f} pp | correct: {} -> {} | nuisance patches={:.4f}%".format(
                            row["class_id"], row["num_images"],
                            row["accuracy_before"], row["accuracy_after"],
                            row["accuracy_gain_pp"],
                            row["num_correct_before"], row["num_correct_after"],
                            row["nuisance_patch_ratio_pct"]
                        )
                    )
                else:
                    print(
                        "[Bank Remove-N] class={} | images=0 | accuracy: N/A -> N/A "
                        "| gain=N/A | correct: 0 -> 0 | nuisance patches=N/A".format(
                            row["class_id"]
                        )
                    )
            for row in per_group:
                if row["num_images"]:
                    print(
                        "[Bank Remove-N] {} | images={} | accuracy: {:.4f}% -> {:.4f}% "
                        "| macro: {:.4f}% -> {:.4f}%".format(
                            row["group"], row["num_images"], row["accuracy_before"],
                            row["accuracy_after"], row["macro_accuracy_before"],
                            row["macro_accuracy_after"]
                        )
                    )
            print("[Bank Remove-N] Test labels were used for metrics only; no threshold search.")
    ...
    def _stage2_triple_bank_partition(
        ...
        x = inputs.to(self._assignment_device, non_blocking=True)
        _, _, features = _capture_layer4_and_forward(self._assignment_model, x)
        regions = self._regions_from_features(features)
        b, r, d = regions.shape
    ...
    def _ce_like_to_class_loss(
        ...
        z_v = F.normalize(z_v, p=2, dim=1, eps=self.eps)
        W = F.normalize(classifier.weight.to(device=z_v.device, dtype=z_v.dtype),p=2,dim=1,eps=self.eps,)
        cosine_logits = z_v @ W.t()
        prior_log = self.prior_log.to(device=z_v.device, dtype=z_v.dtype)
        logits_ce = cosine_logits / self.temperature + prior_log.unsqueeze(0)
        return F.cross_entropy(logits_ce, y_v)
    ...
    def _nuisance_to_class_loss(
        ...
        rotating the classifier. It must reduce dependence carried by Zn.
        """
        logits_n = classifier(z_nuisance)
        prior_log = self.prior_log.to(device=z_nuisance.device, dtype=z_nuisance.dtype)
        uniform_targets = torch.full_like(logits_n, 1.0 / self.num_classes)
        return F.cross_entropy(logits_n+prior_log, uniform_targets)
    # -------------------------------------------------------------------------
    # Public Stage-II forward
    # -------------------------------------------------------------------------
    def forward(
        ...
        logits, output, student_features = _capture_layer4_and_forward(student_model, inputs)
        if logits.shape[1] != self.num_classes:
            raise ValueError(
                "Expected {} classes, got {}.".format(self.num_classes, logits.shape[1])
            )
        ...
        loss_global = self._global_to_class_loss(
            z_global=z_global, z_nuisance=z_nuisance, labels=labels, classifier=classifier
        )
        loss_relevant = self._relevant_region_to_class_loss(
            z_relevant=z_relevant,
            valid_relevant=valid_relevant,
        ...
        loss_nuisance = self._nuisance_to_class_loss(
            z_nuisance=z_nuisance,
            valid_nuisance=valid_nuisance,
            labels=labels,
            classifier=classifier,
        )
        loss_region = loss_nuisance + loss_global + loss_relevant
        return DualPatternOutput(
            loss_region=loss_region,
            loss_global=loss_global,
            loss_relevant=loss_relevant,
    ...
    def total_loss(
        ...
        output: DualPatternOutput,
        lambda_global: Optional[float] = 0.0,
        lambda_relevant: Optional[float] = 1.0,
        lambda_nuisance: Optional[float] = 0.5,
    ) -> Tensor:
        """
        Exact requested three-term objective (no ordinary-logit CE added):
    ...
    def combine_with_classification_loss(
        ...
        classification_loss: Tensor,
        output: DualPatternOutput,
        lambda_relevant: Optional[float] = 0,
        lambda_nuisance: Optional[float] = None,
        lambda_global: Optional[float] = 0,
    ) -> Tensor:
        """
        Backward-compatible helper if you still want the original ordinary CE:
        ...
        return loss, float(lambda_nuisance) * output.loss_region
